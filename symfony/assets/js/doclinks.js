...
document.addEventListener("DOMContentLoaded", function () {
    ...
    modalElt.querySelectorAll(".hljs-comment").forEach((commentElt) => {
        commentElt.innerHTML = commentElt.innerHTML.replace(
            /https:\/\/symfony.com\/[\w/.#-]+/g,
            (url) => anchor(url, url),
        );
    });
    ...
    const attributes = {
        ...
        IsGranted:
            "https://symfony.com/doc/current/security.html#security-securing-controller-annotations",
    };
    ...
    // Wraps Twig's tags
    templateCode
        .querySelectorAll(".hljs-template-tag + .hljs-name")
        .forEach((elt) => {
            ...
            const url =
                "https://twig.symfony.com/doc/3.x/tags/" + tag + ".html#" + tag;
            elt.innerHTML = anchor(url, tag);
        });

    // Wraps Twig's functions
    templateCode
        .querySelectorAll(".hljs-template-variable > .hljs-name")
        .forEach((elt) => {
            const func = elt.textContent;
            const url =
                "https://twig.symfony.com/doc/3.x/functions/" +
                func +
                ".html#" +
                func;
